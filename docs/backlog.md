...
| スラッシュコマンド補完 | `/` 入力でスキル候補をドロップダウン表示、上下キー選択、Tab/Enter 確定 |
| スキル使用バッジ | スキルで送信したメッセージにアンバーバッジ表示 |
| スキル編集・ID 指定・初期化 | カスタムスキルの編集、新規作成時の ID 指定、ビルトインへのリセット |
| MCP セッション未読管理 | 紫ドットで未読表示、開いたら既読 |

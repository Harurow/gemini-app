...
1. **1回のみ登録**: **モジュールレベル変数** (`let registered = false`) でガードする。`useRef` はコンポーネント再マウントでリセットされるため不十分
...
### useEffect 依存配列による再登録問題

App.tsx の `useEffect` に `[setSettingsOpen, loadSessions]` 依存配列があり、これらの参照が変わるたびに effect が再実行され、`ipcRenderer.on` で新しいリスナーが追加されていた。`removeAllListeners` で対処していたが、他コンポーネントのリスナーも巻き込む。

**対策**: App.tsx でも `useRef(false)` ガードを使い、リスナー登録は1回のみ。コールバック内では `useAppStore.getState()` / `useChatStore.getState()` で最新の状態を取得し、クロージャのキャプチャ問題を回避する。
